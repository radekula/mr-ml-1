...
This service is responsible for managing documents in flows.
...
```
docker run -d --name flows-documents-database-mongo -v {host_path}:/data/db mongo
```
...
```
docker run -d -p 8083:8080 --link {database_service_name}:flows-documents-database-mongo --name flows-documents flows-documents
```
